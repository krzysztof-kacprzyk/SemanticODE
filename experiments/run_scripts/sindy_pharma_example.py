...
def create_df_row(sparsity):
    ...
    # Extrapolation
    sindy_benchmark.prepare_data(pk_dataset_extrapolation, train_indices, val_indices, test_indices)
    _, results = sindy_benchmark.evaluate(sindy_model)
...
df.to_latex("analysis/output/Pharma SINDy (Tab 2).tex", index=False)
with open("analysis/output/Pharma SINDy (Tab 2) eqs", "w") as f:
    f.write(eqs)
